...
describe('Resume Structure and Metadata', function () {
  ...
  it('should maintain proper formatting', function (done) {
    ...
    pdfParse(dataBuffer)
      .then((data) => {
        ...
        assert(
          hasDatePattern,
          'Some form of date formatting should be present',
        );
  ...
  it('should have consistent file size with reference', function () {
    ...
    const sizeDifferencePercent =
      (Math.abs(pdfSize - referencePdfSize) / referencePdfSize) * 100;
    assert(
      sizeDifferencePercent < 20,
      `PDF file size differs by ${sizeDifferencePercent.toFixed(2)}% from reference`,
    );
  });
